...
Application.prototype.paginationTerm = 'pagination';
...
Application.prototype.createPage = function(page, schema) {
    ...
    var view = template.compileFile(templateFile);
    var params = {
        appRoot: appRoot
    };
    for (var element in schema) {
        ...
        if (typeof schema[element] === 'object' && element !== self.paginationTerm) {
            params[element] = self.createSeries(element, schema[element], schema.config);
        } else {
    ...
    if (
        typeof schema[self.paginationTerm] !== 'undefined' &&
        typeof schema[self.paginationTerm].count !== 'undefined' &&
        typeof schema[self.paginationTerm].patern !== 'undefined' &&
        typeof params[schema[self.paginationTerm].by] !== 'undefined'
    ) { // pagination:
        var pageBy = _und.clone(params[schema[self.paginationTerm].by]);
        var chunk = [];
        var i = 1;
        while ((chunk = pageBy.splice(0, schema[self.paginationTerm].count)) && chunk.length) {
            var path = schema[self.paginationTerm].patern.replace('{number}', i).replace('{count}', schema[self.paginationTerm].count);
            
            fs.mkdirSync(path.replace(/\/[^\/]*$/, '/'), typeof schema.config.dirMode === 'number' ? schema.config.dirMode : self.defaultDirMode, true);
            log('Save:', path);
            
            params.path = path;
            params.pagination = {
                all: params[schema[self.paginationTerm].by].length,
                courent: chunk.length,
                page: i
            }
            params[schema[self.paginationTerm].by] = chunk;
